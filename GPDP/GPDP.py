...
# ================================== Others ==================================
from GPDP import my_exact_gp
from my_utilities import my_utils, my_NN
from time import time, sleep
import numpy as np
import os

class GaussianProcessDiffusionPolicy:
    def __init__(self, params_dict:dict, dataset:dict, ft:bool=False):
        ...
        # Initialise Paths
        self.training_record_path = 'GPDP/training_records/{:s}_{:s}_training_record'.format(self.ALG, self.ENV_CONFIG)
        self.training_record_2m_path = 'GPDP/training_records/{:s}_{:s}_training_record_2m'.format(self.ALG, self.ENV_CONFIG)
        self.training_checkpoint_path = 'GPDP/training_records/{:s}_{:s}_checkpoint'.format(self.ALG, self.ENV_CONFIG)
        self.evaluation_path = 'GPDP/norm_eval_rewards_append'
        ...
        best_dataset = self.bestTrajExtraction(dataset=dataset)
        self.gp_model = my_exact_gp.myExactGP(params_dict=params_dict, dataset=best_dataset, cuda=CUDA)
    ...
    def bestTrajExtraction(self, dataset:dict):
        _num_samples = dataset['arr_0']
        _state = dataset['arr_1']
        _action = dataset['arr_2']
        _reward = dataset['arr_3']
        _next_state = dataset['arr_4']
        _truncrated = dataset['arr_5']
        _terminated = dataset['arr_6']
        _accum_reward = 0
        _best_reward = 0
        _counter = 0
        for n in range(_num_samples):
            if _truncrated[n] or _terminated[n]:
                if _accum_reward > _best_reward:
                    _best_reward = _accum_reward
                    _last_idx = n
                    _first_idx = n - _counter
                # Reset
                _accum_reward = 0
                _counter = 0
            else:
                _accum_reward += _reward[n]
                _counter += 1
                if _counter >= 1000:
                    # Reset
                    _counter = 0
        ...
        print('best_reward: ', _best_reward, ', first_idx: ', _first_idx, ', last_idx: ', _last_idx)

        _best_dataset = ({'arr_0': len(_state[_first_idx:_last_idx]), 
                          'arr_1': _state[_first_idx:_last_idx], 
                          'arr_2': _action[_first_idx:_last_idx], 
                          'arr_3': _reward[_first_idx:_last_idx], 
                          'arr_4': _next_state[_first_idx:_last_idx], 
                          'arr_5': _truncrated[_first_idx:_last_idx],
                          'arr_6': _terminated[_first_idx:_last_idx]})

        return _best_dataset
    ...
    def reverseProcess(self, inputs:list, size:int, guide:bool=False):
        ...
        for i in range(self.DEC_DIFFU_STEPS): 
            ...
            # Guided term
            if guide:
                _cov_dp = self.beta[rev_pos] * torch.eye(self.ACTION_DIM) if i != (self.DEC_DIFFU_STEPS-1) else 0. * torch.eye(self.ACTION_DIM)
                _cov_gp = torch.clip(self.var_r, min=self.beta[rev_pos]) * torch.eye(self.ACTION_DIM, dtype=torch.float32)
                _inv_cov_gp = torch.linalg.inv(_cov_gp)
                x_t_m_1 += -torch.matmul(torch.matmul(_cov_dp, _inv_cov_gp), torch.squeeze(x_t_m_1 - self.mu_r))
    ...
    def predict(self, state, size:int, guide:bool=True):
        if not torch.is_tensor(state):
            state = torch.tensor(state, dtype=torch.float32).view(-1, self.STATE_DIM)
        return torch.squeeze(self.reverseProcess(state, size, guide))
    ...
    def getAlteredObservation(self, states):
        ...
        return torch.stack(_y_hat_list, dim=0)
